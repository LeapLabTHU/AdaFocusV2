...
class TSN(nn.Module):
    ...
    def _prepare_tsn(self, num_class):
        ...
        if self.new_fc is None:
            ...
            if hasattr(self.new_fc, 'weight'):
                ...
                normal_(self.new_new_fc.weight, 0, std)
                constant_(self.new_new_fc.bias, 0)
        return feature_dim
    ...
    def forward(self, input, glance=False, no_reshape=False):
        if not no_reshape:
        ...
        if self.dropout > 0:
            base_out = self.new_fc(base_out_feat.detach())
            reg_out = self.new_new_fc(base_out_feat)
        ...
        if self.reshape:
            if self.is_shift and self.temporal_pool:
                ...
                reg_out = reg_out.view((-1, self.num_segments // 2) + reg_out.size()[1:])
            else:
                base_out = base_out.view((-1, self.num_segments) + base_out.size()[1:])
                reg_out = reg_out.view((-1, self.num_segments) + reg_out.size()[1:])
            output = self.consensus(base_out)
            reg_output = self.consensus(reg_out)
            if glance:
                return output.squeeze(1), reg_output.squeeze(1), feature_maps.view((-1, self.num_segments * 1280) + feature_maps.size()[2:])
            else:
                return output.squeeze(1), reg_output.squeeze(1)
